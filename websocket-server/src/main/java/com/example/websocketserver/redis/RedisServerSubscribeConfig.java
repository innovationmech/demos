 ...
 */
@Configuration
public class RedisServerSubscribeConfig {
...
//        container.addMessageListener(conListenerAdapter,new PatternTopic("broadcastMsg"));
     ...
     *
     * @param redisServerEvent
     * @return
     */
    @Bean
    MessageListenerAdapter msgListenerAdapter(RedisServerEvent redisServerEvent) {
        // 这个地方 是给messageListenerAdapter 传入一个消息接受的处理器，利用反射的方法调用“receiveMessage”
        // 也有好几个重载方法，这边默认调用处理器的方法 叫handleMessage 可以自己到源码里面看
        return new MessageListenerAdapter(redisServerEvent, "receiveMessage");
    }
